...
uint16 RTS_Table001[SC_RTS_BUFF_SIZE] =
{
  /*  cmd time,  <---------------------------- cmd pkt primary header ---------------------------->  <----- cmd pkt 2nd header ---->   <-- opt data ---> */
     0x0000, CMD1_TIME,  CFE_MAKE_BIG16(SC_CMD_MID), CFE_MAKE_BIG16(PKT_FLAGS), CFE_MAKE_BIG16(CMD1_LENGTH), CFE_MAKE_BIG16((SC_NOOP_CC << 8) | CMD1_XSUM),
     0x0000, CMD2_TIME,  CFE_MAKE_BIG16(SC_CMD_MID), CFE_MAKE_BIG16(PKT_FLAGS), CFE_MAKE_BIG16(CMD2_LENGTH), CFE_MAKE_BIG16((SC_ENABLE_RTS_CC << 8) | CMD2_XSUM), CMD2_ARG, 0x0000, 
     0x0000, CMD3_TIME,  CFE_MAKE_BIG16(SC_CMD_MID), CFE_MAKE_BIG16(PKT_FLAGS), CFE_MAKE_BIG16(CMD3_LENGTH), CFE_MAKE_BIG16((SC_START_RTS_CC << 8) | CMD3_XSUM), CMD3_ARG, 0x0000,
};
